...
export default function UploadCallPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // If SDR, pre-select themselves; owners/managers see all SDRs
      if (prof.role === 'sdr') {
  ...
  if (step === 'analyzing') {
    return (
      <div className="p-8 max-w-lg mx-auto mt-20 text-center">
        <Spinner size="lg" />
        <h2 className="text-lg font-semibold text-gray-900 mt-4">Analyse en cours...</h2>
        <p className="text-sm text-gray-500 mt-1">L&apos;IA analyse la transcription de l&apos;appel.</p>
        <p className="text-xs text-gray-400 mt-1">Cela peut prendre 10 à 20 secondes.</p>
      </div>
    )
  }
