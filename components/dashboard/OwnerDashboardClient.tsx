...
type KpiFilter = 'all' | 'booked' | 'qualified' | 'quality' | null
...
type Props = {
  ...
  bestSdr: SDRLeaderboardRow | null
  weakestSdr: SDRLeaderboardRow | null
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div className="app-scroll" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div className="app-kpi-grid">
          <button className="h3a-kpi-button" type="button" onClick={() => applyKpiFilter('all')} aria-pressed={selectedKpi === 'all'}>
            <StatCard label="Appels analysés" value={kpis.total_calls} className={selectedKpi === 'all' ? 'is-active' : undefined} />
          </button>
          <button className="h3a-kpi-button" type="button" onClick={() => applyKpiFilter('booked')} aria-pressed={selectedKpi === 'booked'}>
            <StatCard label="RDV posés" value={kpis.appointments_booked} className={selectedKpi === 'booked' ? 'is-active' : undefined} />
          </button>
          ...
          <button className="h3a-kpi-button" type="button" onClick={() => applyKpiFilter('quality')} aria-pressed={selectedKpi === 'quality'}>
            <StatCard label="Qualité RDV moy." value={kpis.avg_appointment_quality ?? '—'} sub="/100" className={selectedKpi === 'quality' ? 'is-active' : undefined} />
          </button>
            ...
            <Card style={{ overflow: 'hidden' }}>
              <CardHeader>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>Résumé SDR</h2>
                  <Link href="/coaching" style={{ fontSize: 11, fontWeight: 700, color: 'var(--cyan)' }}>Voir coaching</Link>
                </div>
              </CardHeader>
              <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                <SummaryLine label="Meilleur SDR" value={bestSdr?.sdr_name || '—'} score={bestSdr?.avg_sdr_quality ?? null} />
                <SummaryLine label="SDR à coacher" value={weakestSdr?.sdr_name || '—'} score={weakestSdr?.avg_sdr_quality ?? null} />
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, paddingTop: 10, borderTop: '1px solid var(--border)' }}>
                  <span style={{ fontSize: 12, color: 'var(--muted)' }}>Coaching requis</span>
                  <span style={{ fontSize: 18, fontWeight: 800, color: kpis.sdrs_needing_coaching > 0 ? '#fcd34d' : '#86efac' }}>{kpis.sdrs_needing_coaching}</span>
                </div>
              </div>
            </Card>
                ...
                {orderedCampaigns.map((campaign, index) => (
                  <Link key={campaign.id} href={`/campaigns/${campaign.id}`} style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '13px 18px', borderBottom: index < orderedCampaigns.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, fontSize: 11, color: 'var(--muted)' }}>
                      ...
                      {campaign.totalCalls > 0 ? <Badge className={campaign.health.labelBg}>{campaign.health.label} · {campaign.health.score}</Badge> : <span>Pas de données</span>}
                    </div>
                  </Link>
                ))}
              </div>
...
function SummaryLine({ label, value, score }: { label: string; value: string; score: number | null }) {
  const color = score === null ? 'var(--muted-2)' : score >= 70 ? '#86efac' : score >= 50 ? '#fcd34d' : '#fca5a5'
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '90px minmax(0, 1fr) auto', gap: 10, alignItems: 'center' }}>
      <span style={{ fontSize: 11, color: 'var(--muted-2)', textTransform: 'uppercase', letterSpacing: '.05em', fontWeight: 700 }}>{label}</span>
      <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13, color: 'var(--text)', fontWeight: 700 }}>{value}</span>
