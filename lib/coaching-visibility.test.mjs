...
test('coaching page shows coaching-specific summaries and supervision details', () => {
  ...
  assert.match(page, /Coaching l[eé]ger/, 'summary must distinguish light coaching')
  assert.match(page, /Meilleur appel [aà] partager/, 'best call example should be explicit')
  assert.match(page, /Appel [aà] analyser ensemble/, 'joint-review call example should be explicit')
...
test('coaching competency rows keep progress bars separate from score and trend text', () => {
  ...
  assert.match(page, /gridTemplateColumns: 'minmax\(92px, 122px\) minmax\(0, 1fr\) minmax\(80px, 112px\)'/, 'skill rows should split label, bar, and score/trend into separate columns')
