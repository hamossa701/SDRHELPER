...
test('manager completed dashboard visibility is scoped to assigned SDR team without action assignment gates', () => {
  ...
  assert.match(page, /\.eq\('manager_id', user\.id\)/, 'manager dashboard must load only the current manager team SDRs')
  assert.match(page, /\.in\('sdr_id', teamSdrIds\)/, 'manager KPI aggregation must be scoped to the current manager team calls')
  assert.match(page, /\.eq\('users\.manager_id', user\.id\)/, 'manager dashboard call queries must be scoped to assigned SDRs')
  assert.equal(
    page.includes(".eq('assigned_to'"),
    false,
